...
| Um aparelho por conta | Cada conta só pode ter **um** telemóvel registado, e cada telemóvel só pode estar registado numa conta — é isto que impede alguém de registar o seu aparelho também na conta do colega e picar pelos dois. Ambas as regras são índices únicos na base de dados, não apenas verificações na app. Trocar de telemóvel exige que a gestão remova o antigo. **O que não é possível:** o WebAuthn nunca diz ao servidor *que* dedo foi usado, e um telemóvel pode ter várias digitais configuradas nas definições do sistema — a digital prova posse de um aparelho desbloqueado, não identidade. É por isso que a amostragem de selfies se mantém. O ☁️ na lista marca chaves sincronizadas na conta iCloud/Google do próprio (podem funcionar noutro aparelho dele, nunca de outra pessoa). Migração: `2026-08-20_um_dispositivo_por_conta.sql`. |
