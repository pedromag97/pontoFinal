...
| Registos offline | Sem rede, o registo (selfie+GPS) fica no IndexedDB do telemóvel e sincroniza sozinho quando a ligação volta. Sem servidor no momento da picagem, a **data** vem do telemóvel — e só é aceite se for plausível: **nada no futuro e nada com mais de 7 dias**. Fora dessa janela a data é recusada, o registo fica no dia do servidor e leva a marca "Data do telemóvel recusada". Todos os registos offline levam a marca "Enviado offline" e **nunca são validados automaticamente**. Limitação assumida: a hora gravada é a da sincronização, não a da picagem. O service worker serve a app do cache para ela abrir offline. |
| Impressão digital (WebAuthn) | O funcionário regista o telemóvel uma vez (banner na app) e passa a picar com a digital. **Nenhum dado biométrico chega ao servidor**: a digital desbloqueia uma chave guardada no aparelho e o servidor só verifica a assinatura — juridicamente é autenticação, não tratamento de biometria. Prova que aquele telemóvel está presente e foi desbloqueado; não substitui a selfie na prova de identidade, por isso a selfie continua a ser exigida onde conta (ver linha seguinte). Admin vê quem tem telemóvel registado e remove-o na troca de aparelho. Migrações: `2026-08-14_impressao_digital.sql` (antes do deploy) e `2026-08-14_impressao_digital_fechar.sql` (depois). |
| Um aparelho por conta | Cada conta só pode ter **um** telemóvel registado, e cada telemóvel só pode estar registado numa conta — é isto que impede alguém de registar o seu aparelho também na conta do colega e picar pelos dois. Ambas as regras são índices únicos na base de dados, não apenas verificações na app. Trocar de telemóvel exige que a gestão remova o antigo — em Gestão › Funcionários, botão 👆. O funcionário **não** pode limpar o seu próprio registo (a política de RLS só permite a admins); se pudesse, a regra "uma conta, um aparelho" não valia nada. **O que não é possível:** o WebAuthn nunca diz ao servidor *que* dedo foi usado, e um telemóvel pode ter várias digitais configuradas nas definições do sistema — a digital prova posse de um aparelho desbloqueado, não identidade. É por isso que a amostragem de selfies se mantém. O ☁️ na lista marca chaves sincronizadas na conta iCloud/Google do próprio (podem funcionar noutro aparelho dele, nunca de outra pessoa). Migrações: `2026-08-20_um_dispositivo_por_conta.sql` e `2026-08-20_so_gestao_limpa_telemovel.sql`. |
| Avisar quem falta registar | Botão 👆 em Gestão › Funcionários: envia notificação a todos os funcionários ativos **sem telemóvel registado**, com o número no próprio botão. Quem já registou não é incomodado. O resultado nomeia quem **não** tem notificações ativas — esses não recebem nada e têm de ser avisados à mão, e sem os nomes o resultado não serviria de nada. Sem telemóvel registado, a política exige selfie em todas as picagens, e quem dispensou o convite raramente percebe que é isso que lhe está a acontecer. |
| Selfie por exceção | Deixou de ser pedida em todas as picagens. É exigida quando: o registo cai **fora das obras**, é **offline**, o funcionário **ainda não registou telemóvel**, por **amostragem aleatória (1 em 10)**, ou porque a **gestão a pediu**. Corta ~90% das fotos guardadas — o ponto mais pesado no RGPD — mantendo o efeito dissuasor. A decisão é do **servidor** (rota `/api/registo/challenge`) e fica gravada no desafio: um cliente manipulado não a pode contornar. |
| Aviso "a app não me deixa registar" | Botão sempre visível na app do funcionário. É a válvula de escape, por isso é deliberadamente pobre em requisitos: **não precisa de GPS, câmara, digital nem desafio** — só de sessão e de rede, porque é precisamente quando o resto falha que faz falta. O servidor junta o contexto técnico sozinho (telemóvel registado ou não, qual a credencial, as últimas tentativas perdidas), para a gestão não ficar dependente do que a pessoa souber explicar, e notifica os admins com push ativo. Aparece em Registos, num painel vermelho no topo, **sem filtro de período** — um funcionário encravado é urgente agora, não no mês que se anda a ver. Migração: `2026-09-20_avisos_app_encravada.sql`. |
| Saída por selfie quando a digital falha | Uma chave de acesso pode desaparecer do telemóvel sem ninguém a apagar (passkeys sincronizadas: basta uma reposição do gestor de palavras-passe ou sair da conta Google). Antes, o servidor continuava a exigir a assinatura que o aparelho já não sabia dar e o funcionário ficava **trancado fora da app** — aconteceu, e custou dois dias de registos. Agora, quando `startAuthentication` falha, a app oferece **"Registar com selfie"**: pede um desafio novo com `sem_digital`, o servidor emite-o com `fingerprint_waived` e aceita o registo sem assinatura **a troco da foto obrigatória**. O registo leva a marca "Sem digital — selfie no lugar", conta como suspeito e **nunca é validado automaticamente**. A troca é deliberada: a selfie é prova de identidade mais forte que a posse do aparelho, e perder a picagem seria pior do que revê-la. Migração: `2026-09-20_saida_por_selfie.sql`. |
| Selfie a pedido da gestão | A gestão pode exigir foto no próximo movimento de um funcionário: botão 📸 em Gestão › Funcionários, com motivo registado. Acontece também **automaticamente ao recusar um registo** — sem isso, recusava-se por suspeita e o registo refeito vinha sem foto em 9 de 10 casos, que era precisamente a prova que se queria. O pedido é de **uso único**: consome-se na picagem seguinte que traga foto e desliga-se sozinho, senão ao fim de uma semana metade da equipa estaria a tirar foto sempre. O funcionário **não fica a saber** que a foto lhe foi pedida — a app diz-lhe o mesmo que diz numa amostragem, e o motivo verdadeiro nunca sai do servidor; caso contrário, quem tentasse aldrabar sabia exatamente quando ter cuidado. Anular a recusa apaga o pedido que ela criou (um pedido feito à mão fica). Migrações: `2026-08-20_selfie_a_pedido.sql` e `2026-08-21_pedido_selfie_origem.sql`. |
| Registos criados pelo servidor | O cliente deixou de inserir registos diretamente na base de dados (a política RLS de INSERT foi removida). Tudo passa por `/api/registo/entry`, que confere o desafio, a assinatura da digital e a presença da foto antes de gravar. |
